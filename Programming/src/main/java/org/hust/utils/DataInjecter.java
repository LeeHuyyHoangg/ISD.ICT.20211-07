...
public class DataInjecter {
    ...
    public static void injectSation() throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        MongoCursor<Document> cursor = db.getCollection("bikes").find().iterator();
        while(cursor.hasNext()){
            Document bike = cursor.next();
            Bike bike1;
                ...
                for(Class<? extends Bike> c : list) {
                    if(bikeType.equals(c)) {
                        ...
                        switch (random.nextInt(4)){
                            case 0: bikes0.add(bike1.get_id().toHexString());
                            break;
                            case 1: bikes1.add(bike1.get_id().toHexString());
                            break;
                            case 2: bikes2.add(bike1.get_id().toHexString());
                            break;
                            default: bikes3.add(bike1.get_id().toHexString());
                            break;
                        }
                    }
                }
